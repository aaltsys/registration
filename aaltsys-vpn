...
# Default-Stop:      0 1 6
# Short-Description: AAltSys VPN service
### END INIT INFO
...
# Modifid for aaltsys by Michael Cochran
...
DESC="aaltsys virtual private network daemon"
...
start)
  ...
  if test -e $CONFIG_DIR/$NAME.conf ; then
    log_daemon_msg "  Autostarting AAltSys VPN"
    start_vpn
  else
    log_failure_msg "  Autostarting AAltSys VPN: missing $CONFIG_DIR/$NAME.conf file !"
    STATUS=1
  fi
  exit ${STATUS:-0}
  ;;
stop)
  log_action_begin_msg "Stopping AAltSys VPN"
  PIDFILE=
  for PIDFILE in `ls /var/run/openvpn.client.pid 2> /dev/null`; do
    NAME=`echo $PIDFILE | cut -c18-`
    NAME=${NAME%%.pid}
    log_daemon_msg "  Stopping AAltSys VPN '$NAME'"
    stop_vpn
  done
  if test -z "$PIDFILE" ; then
    log_warning_msg "  No AAltSys VPN is running."
  fi
  ;;
